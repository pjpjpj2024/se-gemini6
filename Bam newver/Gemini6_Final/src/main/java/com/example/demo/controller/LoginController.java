...
public class LoginController {
    ...
    //
    // @PostMapping("/login")
    // public String login(Model model, @RequestParam String username, @RequestParam
    // String password) {
    ...
    // bam
    // @PostMapping("/login")
    // public ResponseEntity<?> login(@RequestParam String username, @RequestParam
    // String password, HttpSession session) {
    // // Here you can implement your custom authentication logic (check the
    // database, etc.)
    // User foundUser = userRepository.findByUsername(username);

    // if (foundUser != null && foundUser.getPassword().equals(password)) {
    // // If the user exists and the password matches, store the username in session
    // session.setAttribute("username", username); // Save the username to the
    // session
    ...
    // return ResponseEntity.status(401).body(Map.of("error", "Invalid username or
    // password"));
    // }
    // }
